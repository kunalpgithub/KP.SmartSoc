...
@Injectable()
export class SocietyServiceProxy {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
     ...
     * @return Success
     */
    get(id: string | undefined): Observable<SocietyDto> {
        let url_ = this.baseUrl + "/api/services/app/Society/Get?";
        if (id === null)
            throw new Error("The parameter 'id' cannot be null.");
        else if (id !== undefined)
            url_ += "Id=" + encodeURIComponent("" + id) + "&";
    ...
    protected processGet(response: HttpResponseBase): Observable<SocietyDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
...
export class SocietyDto implements ISocietyDto {
    ...
    static fromJS(data: any): SocietyDto {
        data = typeof data === 'object' ? data : {};
        let result = new SocietyDto();
        result.init(data);
        return result;
    }
